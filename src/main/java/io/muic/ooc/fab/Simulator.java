...
public class Simulator {
    ...
    public Simulator(int depth, int width) {
        ...
        for (int i=0;i<humanTypes.length;i++) {
            view.setColor(humanTypes[i].getHumanClass(), humanTypes[i].getColor());
        }
        reset();
    }
    ...
    public void simulateOneStep() {
        ...
        for (Iterator<Human> it = human.iterator(); it.hasNext();) {
            Human human = it.next();
            human.act();
        }
...
//        view.showStatus(2, field);
    ...
    public void reset() {
        step = 0;
        animals.clear();
        new FieldPopulator().populate(field, animals, human);
